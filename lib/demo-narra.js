...
function updateTitle() {
    ...
    console.log("eccolo   " + narra.items)
...
function forceSaveObjectToDB(objList, objID) {
	...
	//FARE L'INSERT QUI: SCORRERE narra.items (objList), trovare quelo che ha id objID, e inserrlo nelle entità 
 	$.ajax({
			type: "POST",
			url: "../PHP/insertDb.php", 
			dataType: "JSON",					
    ...
    $.getJSON(sparqlURL, function (data) {
        ...
        for (var i=0; i < qids.length; i++) {
            ...
            if (i == qids.length - 1 && narra.counter == lastRun) {
				console.log("aggiorn titolooooooooooooooooooooooooo")
				console.log(qids)
                updateTitle();
                if (callback !== undefined) {
