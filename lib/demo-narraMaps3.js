...
var narra = {
    ...
    // Name of the current user (for creating table in db: for purs users it's the same of userNameToDisplay; for VRE users it's different)
    user: "",
	
    // Name of the current user (for displaiyng in menu)
    userNameToDisplay: "",
...
// create HTML select for switch position of a event
function createHTMLSelect(){
	...
	// append firs blank line
	$('#positionEvent').append('<option value="empty" ></option>')
...
function switchPositionEvent(afterBefore, eventToSwitch){
	...
	// get position of event with which i want to switch my new or modifyed event
	var position= parseInt(narra.events[eventToSwitch].position)
	...
	if(afterBefore == "before"){
		...
		for(var i=0; i < eventsList.length; i++){
			...
			if (eventsList[i] == eventToSwitch){toSwitch = true}
...
// fix order position of events after a switch or a delete of a event
    ...
    $(document).ready(function() {        
		...
		$("#eventImageInput").on("change", function(){
			...
			if($(this).prop('files')[0] != undefined){
				...
				if ($.inArray(fileType, validImageTypes) < 0) {
					...
					$("#selectEventLocalImage").empty()
					$("#selectEventLocalImage").text("Select Image")
					$("#eventImageInput").val("")							 

					showModal(
						"Invalid file",
						"Please select a valid image (png/jpeg)",
						"Back",
						"Confirm",
						function() {
							return
				...
				} else if($(this).prop('files')[0].size > 2000000){
					...
					showModal(
						"Image too large",
						"Please select a smaller image (Max 2 MB)",
						"Back",
						"Confirm",
						function() {
							return
					...
					// file ok
					var imgName= $(this).prop('files')[0].name
					$("#selectEventLocalImage").empty()
					$("#selectEventLocalImage").text("Select Image: " + imgName)				
        ...
        */
		...
		//set inout to search event bar
			// Al cambiamento del testo nell'input
		  $("#eventSearch").on("input", function() {
			// Recupera il testo dell'input
			var searchText = $(this).val().toLowerCase();

			// Recupera tutti i div con classe "contenuti" nel div "contenitore"
			$(".timelineEvent").each(function() {
			  // Recupera il titolo del div corrente
			  var title= $(this).find("h3").text().toLowerCase();

			  // Mostra il div se il suo titolo contiene il testo dell'input, altrimenti nascondilo
		...
		success: function(resp) {
			console.log(resp.username)
				...
				if(resp.username != ""){
					...
					// init DB with userame (NOT usernameToDisplayInMenu) + subject narrative (dbName)
...
//dbName is userName-idWikidata; idNarration is id of narration. It will created in php for new narrations, it passed in query string for existing narrations
function initDB(dbName, idNarration) {
	...
	$.ajax({
		...
		success: function(resp) {
			...
			// assign user (for table creation, NOT to disply in menu) to narra object
			...
			// tutto questo non serve piu. FARE dbNameRequest degli id wikidata (toUppercase)) e titoli passati dal php
/* 			var DBsToLoad = [];
	...
	$.ajax({
			...
			success: function(resp) {
				...
				if(resp.title == ""){
					...
					//change title in MY NARRAIVES MENU
					$(".activeMenu a").text(/* makeTitle() +  */ titlecase(narra.items[narra.subjectID].enName));
				...
				// after a title modification, title will be this title				
					...
					//change title in MY NARRAIVES MENU
					$(".activeMenu a").text(/* makeTitle() +  */ resp.title);
        ...
        */
        ...
        if (missingData) return undefined;
			...
			if (narra.events[evid] != undefined) {
				...
				} else {var position= parseInt(narra.events[evid].position);}
		...
		var imageEventUrl= $("#eventMedia").val().trim()
		var localImageEvent = $('#eventImageInput').prop('files');
		
		if(localImageEvent.length > 0){
			imageEventUrl= "https://tool.dlnarratives.eu/images/"+narra.id_n +"-"+narra.user+"-"+evid+"."+ localImageEvent[0].type.split("/")[1]
...
//upload a image of a event
function uploadEventImg(localImage, eventId){
...
function loadEvent(evid) {
	...
	// if digital oject is array of objrcts (title + url)
	...
	// if digital oject are old djob (only array of urls)
...
function wikipediaRequest(url, lang, linksArray, subjectName) {
    ...
    if (url.indexOf("pageimages") < 0) $.getJSON(wikipediaURL, function (links) {
		...
		// search in wikipedia otherlanguage if there arent't results in current language
		} else {
    ...
    $.getJSON(sparqlURL)
		.done(function (data) {
					...
					if (i == qids.length - 1 && narra.counter == lastRun) {
						...
						if (callback !== undefined) {
							...
							*/
							...
							console.log(qids)
							console.log("qids.lenght è > 0 ?: " + qids.length)
							if (qids.length > 1) {
			...
			// else if sparql query is empty...
				...
				// delete narration previusly inerted in db and shows error
				$.ajax({
					type: "POST",
    ...
    narra.slides = events.map(
        function (key) {
			...
			function createLinkWithTooltip(qid, lang) {
					...
					if(qid.startsWith("Q")){
							var linkIri= "https://www.wikidata.org/wiki/" + narra.items[qid]['_id']
					} else {
						var linkIri = "https://tool.dlnarratives.eu/CustomEntity/?idU="+qid+"&user="+narra.user+"&idN="+narra.id_n+"&subject="+narra.subjectID
					}
...
function visualizeMap(eventsOfNarration) {
		...
		$('#mapdiv').show();
		$('#footer').show();
		// display button publishMap in table, and position near "useraname" button
...
/*  				var dateStory= dateInterval(event.start, event.end);
				if(dateStory != ""){
					 var dateToAppend = "<span class='storymap-date'>"+dateStory+"</span>"
				...
				event.text = {
					'headline': event.title,
					//'text': event.description ? dateToAppend + event.description : ""
					'text': event.description ? event.description : ""
				};
...
/* 					var dateStory= dateInterval(event.start, event.end);
					if(dateStory != ""){
						 event.text.text = "<span class='storymap-date'>"+dateStory+"</span><br/>" + event.text.text + textToAppend
...
function publishNarration(){
	...
	var slidesObject= {"slides": narra.slides, "items" : narra.items, "events": narra.events}
	...
	$.ajax({
		...
		success: function(resp) {
			...
			if(window.location.host=="dlnarratives.moving.d4science.org"){
				var searchOtherNarratives="<br/><br/>Find all stored narratives: <input type='text' value='https://dlnarratives.moving.d4science.org/Search/' readonly>"
			} else {
				var searchOtherNarratives="<br/><br/>Find all stored narratives: <input type='text' value='https://tool.dlnarratives.eu/Search/?dataset=narratives' readonly>"
			}
...
function uploadImage(element, blob, fileName) {
	...
	xhr.onreadystatechange = function() {
	    ...
	    if (xhr.readyState == XMLHttpRequest.DONE) {
	        ...
	        replaceImage($(element), "../images/" + fileName);
...
function createDigObjPreview(urlObj, title) {
    ...
    if ($("#digobjTable .digobjPreview[data-url='" + urlObj + "']").length === 0) {
        ...
        var $preview = $("<a target='_blank' href='" + urlObj + "'><div class='digobjPreview' data-url='" + urlObj + "'><div class='deleteButton digObjDeleteButton'><b class='x'>×</b></div><span>"+title+"</span></div></a>");
...
// Perform SPARQL request for each entity
function dbNameRequest(qids, title) {
    //console.log(currentTime() + "Wikidata request");
...
function openPopupMap(){
	...
	var popup = window.open("HTML/searchCoordinates.html", "Search_Coordinates", "width=900,height=700");
...
/* function formatDateForStorymap(dateIntervall){
	
	if(dateIntervall.includes("–")){
		
		var arrayDate= dateIntervall.split("-")
