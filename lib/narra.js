...
window.onload = function () {
	...
	dbName = dbName ? dbName : $('meta[name=database]').attr("content");
	
	// Open PouchDB remote database
	narra.remote = new PouchDB("https://" + window.location.host + "/db/" + dbName, {skipSetup: true, cache: true});
	...
	narra.remote.getSession(function (err, response) {
		if (err) {
			...
			.then(function (result) {
				...
				$("#lang,#userMenu").text(narra.user.toUpperCase()).show();
				$("#auth-div").hide();
				$("#container").show();
...
function syncDB() {
    ...
    .on('complete', function() {
		...
		narra.db.sync(narra.remote, {live: true, retry: true})
		.on('error', console.log.bind(console))
	});*/
}
...
function deleteNarrative(dbid) {
	narra.remote.getUser(narra.user).then(function (user) {
		delete(user.narratives[dbid]);
		narra.remote.putUser(narra.user, {metadata: {narratives: user.narratives}}).then(function (response) {
			console.log(response);
			showUserMenu();
		})
		.catch(function (err) {
...
function typeFromArray(array) {
    ...
    if (array.indexOf("Q7184903") > -1 || array.indexOf("Q4026292") > -1) {
        return "concept";
    }
...
function startQueries() {
	...
	narra.db.allDocs({
		...
		console.log(currentTime() + "Loaded " + result.rows.length + " items from database");
	...
	narra.db.get("A1").then(function (doc) {
		...
		narra.info = {
			_id: "A1",
			id: $('meta[name=narraID]').attr("content"),
			name: "",
			author: $('meta[name=narraAuthor]').attr("content")
		};
		saveObjectToDB({"A1": narra.info}, "A1");
	})
	
	// Load relations from database
	narra.db.get("D1").then(function (doc) {
		if (doc !== undefined) {
			narra.rels = doc.rels;
		}
		console.log(currentTime() + "Loaded relations from database");
	}).catch(function (err) {
		console.log(currentTime() + 'Relations not found in database');
		narra.rels = {
			_id: "D1",
		};
		saveObjectToDB({"D1": narra.rels}, "D1");	
	});
	...
	narra.db.get("B1").then(function (doc) {
		...
		console.log(currentTime() + 'Bibliography not found in database');
	});
	*/
	$("#auth-div").hide();
}
...
function makePopoverContent(self, withForm, evid) {
    ...
    var $imageDiv = $("<div class='popoverContent popoverCenter lowerMargin'>").append(
        "<div class='popoverDiv'>" +
				"<div class='image-upload-spinner' style='display: none'>\
					<div class='f_circleG image-upload-circle' id='frotateG_01'></div>\
					<div class='f_circleG image-upload-circle' id='frotateG_02'></div>\
					<div class='f_circleG image-upload-circle' id='frotateG_03'></div>\
					<div class='f_circleG image-upload-circle' id='frotateG_04'></div>\
					<div class='f_circleG image-upload-circle' id='frotateG_05'></div>\
					<div class='f_circleG image-upload-circle' id='frotateG_06'></div>\
					<div class='f_circleG image-upload-circle' id='frotateG_07'></div>\
					<div class='f_circleG image-upload-circle' id='frotateG_08'></div>\
				</div>" +
...
function makeDataDiv(qid) {
    if (narra.items[qid] !== undefined) {
        ...
        return "<a class='data " + type + "' role='button' tabindex=-1 data-id='" + qid + "' data-class='" + type + "' style='background-color: " + getColor(type) + "'" +
                "onclick='dataDivClick($(this))' data-name='" + getLabel(qid).toLowerCase() + "'>" +
                truncate(capitalize(getLabel(qid)), Math.floor(($("#controls").width() / 10) * parseInt($("body").css("font-size").replace("px", "")) / 36)) + "</a>";
    } else {
        return false;
    }
}
...
function resetWorkspace() {
	...
	$("#workspace .form-group, #workspace .form-inline, #workspace .input-group").removeClass("has-error");
...
function deleteEntity(qid) {
	...
	showModal(
		"Delete Entity",
		"Are you sure you want to delete the entity \"" + title + "\"? It will also be deleted from all corresponding events.",
		"Keep Entity",
		"Delete Entity",
		function() {
		},
		function() {
			...
			// Delete entity from all events
			for (var evid in narra.events) {
				if (qid in narra.events[evid].props) {
					delete narra.events[evid].props[qid];
...
function deleteObjectFromDB(objID) {
	narra.db.get(objID).then(function(doc) {
    	...
    	console.log(currentTime() + " Error deleting " + objID + " -- "+ err.name);
	});
}
...
function getObjectFromDB(objID) {
	narra.db.get(objID)
	.then(function(doc) {
	    return true;
	})
	...
	return new Promise(function(resolve) {
		...
		.catch(function (err) {
			if (err.reason === "missing" || err.reason === "deleted") {
				...
				.catch(function (err) {
					narra.online = false;
					alert("Error saving " + objID + " -- " + err.reason);
					resolve(false);
				});
			}
			else {
				narra.online = false;
				alert("Error! Either your Internet connection is offline or your session has expired. Please export your data, then reload the page and try again.");
				resolve(false);
			}
		});
	});
...
function saveEvent(data) {
    ...
    if (data == undefined) {
		...
		*/
        ...
        if ($("#inputDiv").find(".data").length === 0) {
            $("#inputDiv").parents('.form-group, .form-inline').addClass("has-error");
            missingData = true;
        };
	...
	saveObjectToDB(narra.events, evid).then(function(result) {
		if (result) {
			displayEvent(undefined, evid);
			resetWorkspace();
		}
	});
...
function displayAllEvents() {
	...
	console.log(Object.keys(narra.events).length);
...
// TODO Check event display logic
...
function makeQuery(qids, isSubject, force) {
	...
	"wd:Q15474042 wd:Q41176 wd:Q8205328 \n}";
...
// Ask the user to save the narrative before visualizing the timeline
function confirmVisualize() {
	if (!workspaceIsEmpty()) {
		...
		// Dynamically load timeline CSS to speed up initial load
		if ($("link#timeline-css2").length < 1) {
			var timelineCSS = document.createElement('link');
			timelineCSS.rel = 'stylesheet';
			timelineCSS.href = '/lib/timeline.css';
			timelineCSS.type = 'text/css';
			timelineCSS.onload = function() { visualize(); }
			var mainCSS = document.getElementsByTagName('link')[0];
			mainCSS.parentNode.insertBefore(timelineCSS, mainCSS.nextSibling);
		}
		else {
			visualize();
		}
...
function visualize() {
	...
	narra.slides = events.map(
		function (key) {
			...
			event.order = narra.events[key].order || null;			
			
			if (event.title.indexOf("IPCC SRES") > -1) event.order = 1;
			if (event.title.indexOf("RCP 4.5") > -1) event.order = 2;
			if (event.title.indexOf("RCP 8.5") > -1) event.order = 3;
			if (event.title.indexOf("Climate Snapshot") > -1) event.order = 4;
			if (event.title.indexOf("Medium Concentration of Greenhouse Gases") > -1) event.order = 10;
			if (event.title.indexOf("High Concentration of Greenhouse Gases") > -1) event.order = 11;
	...
	var observer = new MutationObserver(function (m) {
		if(m && m.length > 0 && m[0].addedNodes && m[0].addedNodes.length > 0 && (m[0].addedNodes[0].nodeName === "#text" || m[0].addedNodes[0].nodeName == "IMG")) {
	  	  	makeImageSelect(window.timeline.getSlide(0).data.unique_id);
			$(".tl-media-image").each(function(k, v) {
				$(this).magnificPopup({type: 'image', items: {src: $(v).attr('src')}, closeOnContentClick: true});
			});
			this.disconnect();
	  	}
	});
	...
	.on("change", function(data) {
		...
		$("#" + data.unique_id + " .tl-media-image").delay(500).each(function(k, v) {
			$(this).magnificPopup({type: 'image', items: {src: $(v).attr('src')}, closeOnContentClick: true});
		});
	});
}
...
function switchImage(value, slideID) {
	...
	saveObjectToDB(narra.events, evid);
	$("#" + slideID + " " + ".tl-media-image")
	.attr("src", value)
	.magnificPopup({type: 'image', items: {src: value}, closeOnContentClick: true});
}
...
function searchEntity() {
	...
	$("#controls .data[data-name*='" + $("#searchInput").val().toLowerCase() + "']").show();
...
function confirmLogout() {
	showModal(
		"Logout",
		"Are you sure you want to logout? You will lose all unsaved data.",
		"Logout",
		"Don't Logout",
		function() {
	...
	$.get("https://dlnarratives.eu/cgi-bin/triplify.py?db=" + narra.db.name.split('/').pop(),
		function(data) {
			...
			showModal(
				...
				data.result.indexOf("inconsistent") > -1 ? data.result : "The ontology is consistent.<br><br>" +
				    "The narrative has been published here: <a target='_blank' href='/timeline/" + narra.db.name.split('/').pop() +
				    ".html'>" + narra.info.name + "</a>.<br><br><a target='_blank' href='" + data.path +
				    "'>Download the OWL file of your narrative</i>",
				"Cancel",
				"OK",
				function() {},
				function() {}
