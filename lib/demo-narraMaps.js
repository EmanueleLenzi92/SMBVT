...
/* 			var DBsToLoad = [];
               ...
               for(var i = 0; i < resp.allDBName.length; i ++) {
				...
				for(var i = 0; i < resp.allDBName.length; i ++) {
					...
					$("#narratives-menu").append("<li class='narra-list-item'><a href='tool.html?idwiki=" + resp.allDBName[i][2].toUpperCase() + "&idnar="+resp.allDBName[i][0]+"' class='data' target = '_blank'>" + resp.allDBName[i][1] + "</a></li>");
...
/*         if (!$("#typeInput").val()) {
        ...
        narra.events[evid] = {
			...
			"position" : position,
			"mapMarkerColor" : $('#markerMapColor option').filter(':selected').val()
        };
    }
    else {
    ...
    // Save event to Postgres database
    ...
    $.getJSON(sparqlURL)
		.done(function (data) {
			...
			// else if sparql query is empty...
				...
				showModal(
				"Entity not found",
				"there is no instance of this entity in wikidata. Search another entity",
				"Back",
				"Confirm",
				function() {
...
function visualizeMap(eventsOfNarration) {
		...
		$('#mapdiv').show();
		$('#footer').show();
		//$("#buttonPublishMap").show()
		$("#buttonPublishMap").css("display","table");
		$('#topTitle').css("margin-bottom", "2%");
...
function publishMap(){
	...
	var title= $('#bigName').text()
	var slidesObject= {'storymap': {'slides' : narra.slides}}
	

	$.ajax({
		type: "POST",
		url: "../PHP/publishMap.php", 
		dataType: "JSON",					
		data: {user : narra.user, narrationTitle : title, slides : JSON.stringify(slidesObject), idNarr: narra.id_n, subject: narra.subjectID },
		success: function(resp) {
			
		    showModal(
				"Storymap published",
				"The storymap has been published<br/><br/>Link: <a href='"+resp.link+"' target='a_blank'>" + resp.link + "</a>",
				"back",
				"ok",
				function() {
...
function loadImageFromFile(element) {
	...
	if (file) {
		if (file.type.match(/image.*/)) {
			...
			var fileName = narra.user  + "-image-" + Math.floor(Math.random() * 10000000) + "." + extension;
...
function startTriplify() {
   // $.get("https://dlnarratives.eu/cgi-bin/triplify.py?db=" + narra.db.name.split('/').pop(),
	...
	$.ajax({
			...
			dataType: "JSON",					
			data: {currentDbName: narra.id_n + narra.user +"-"+narra.dbName},
			success: function(resp) {
